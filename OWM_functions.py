...
def get_text_forecast(today: pd.DataFrame, past_forecast: pd.DataFrame, past_real: pd.DataFrame) -> str:
    ...
    6. ВАЖНОЕ ПРАВИЛО ФОРМАТИРОВАНИЯ ТЕКСТА:
        Для выделения температуры, осадков и важных цифр используй ТОЛЬКО HTML-теги <b> и </b>.
        КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать Markdown и двойные звездочки (**). Telegram их не поймет.

        ✅ ПРАВИЛЬНО: Утром <b>+2°C</b>, ветер <b>5 м/с</b>.
        ❌ НЕПРАВИЛЬНО: Утром **+2°C**, ветер **5 м/с**.
    '''
